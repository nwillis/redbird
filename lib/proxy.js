...
function ReverseProxy(opts) {
  ...
  if (opts.cluster && cluster.isMaster) {
    ...
    // Add customized response headers
    proxy.on('proxyRes', function (proxyRes, req) {
      var src = _this._getSource(req);
      if (headersList[src]) {
        for (var key in headersList[src]) {
          proxyRes.headers[key] = headersList[src][key];
        }
      }
    });
...
ReverseProxy.prototype.register = function (src, target, opts) {
  ...
  if (opts) {
    ...
    // Get customized headers on response
    if (opts.res_headers) {
      var s_src = src.hostname;
      if (s_src) {
        headersList[s_src] = opts.res_headers;
      }
    }
  }
...
ReverseProxy.prototype._getTarget = function (src, req) {
  ...
  return this.resolve(src, url, req).bind(this).then(function (route) {
    ...
    this.log && this.log.info('[%s]Proxying %s to %s', req.connection.remoteAddress, src + url, path.join(target.host, req.url));
